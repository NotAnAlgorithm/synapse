...
addons-failed-to-load2 =
    ...
    They may need to be updated to support this version of Synapse (Anki). Click the { addons-check-for-updates } button
...
addons-changes-will-take-effect-when-anki = Changes will take effect when Synapse is restarted.
...
addons-download-complete-please-restart-anki-to = Download complete. Please restart Synapse to apply changes.
...
addons-install-anki-addon = Install Synapse (Anki) add-on
...
addons-invalid-code-or-addon-not-available = Invalid code, or add-on not available for your version of Synapse (Anki).
...
addons-packaged-anki-addon = Packaged Synapse (Anki) Add-on
...
addons-please-restart-anki-to-complete-the = <b>Please restart Synapse to complete the installation.</b>
...
addons-this-addon-is-not-compatible-with = This add-on is not compatible with your version of Synapse (Anki).
...
addons-unable-to-update-or-delete-addon = Unable to update or delete add-on. Please start Synapse while holding down the shift key to temporarily disable add-ons, then try again.  Debug info: { $val }
