...
importing-by-default-anki-will-detect-the = By default, Synapse will detect the character between fields, such as a tab, comma, and so on. If Synapse is detecting the character incorrectly, you can enter it here. Use \t to represent tab.
...
importing-merge-notetypes-help =
    If checked, and you or the deck author altered the schema of a note type, Synapse will
...
importing-packaged-anki-deckcollection-apkg-colpkg-zip = Packaged Synapse (Anki) Deck/Collection (*.apkg *.colpkg *.zip)
# the '|' character
importing-pipe = Pipe
...
importing-field-separator-help =
    ...
    If a file header is not present, Synapse will try to guess what the separator is.
